...
void Task_Act(void *parameter)
{
  ...
  DB.Preheat = 0;
  while(DB.Preheat++ <= 380)          //38秒后关机
  {
    if(DB.Preheat == 100) Escape(1);
    if(DB.Preheat == 110) Pump(1);
    if(DB.Preheat == 260) Pump(0);
    vTaskDelay(100);
  }
  Pump(0);
  Escape(0);
...
void Task_Tim(void *parameter)
{
  ...
  while(1)
  {
    if(DB.Para[3] < 3) DB.Para[3] = 3;
    if(DB.Para[4] < 3) DB.Para[4] = 3;
		
    if(DB.HandTimer >= DB.Para[3] + DB.Para[4] + DB.WaitTime)
    {
      DB.HandTimer = 0;
      DB.WaitTime = 0;
      DB.CountFlag = 1;
    ...
    if(DB.HandTimer > DB.Para[3] && DB.HandTimer < DB.Para[3] + DB.Para[4] + DB.WaitTime)
    {
      if((DB.State == 1 || DB.State == 3 || DB.State == 8 || DB.State == 9)
         && DB.PressVal > 80 && DB.PressVal < 100 && DB.WaitTime < 3 && DB.Para[0] == 1 && DB.Step <= 2)
      {
        DB.WaitTime++;
        DB.Step = 2;
      }
      ...
      {
        if(DB.HandTimer == DB.Para[3] + 1 + DB.WaitTime)
        {
          DB.HandState = 2;  //伸直
          DB.Step = 3;
        }
    ...
    tmp = (0x06EE - ADC_Val[TMP_ADC])/0x05 + 25;
    if(tmp > -50 && tmp < 100) 
    {
      DB.ChipTmp = tmp*1/4 + DB.ChipTmp*3/4;
    }
...
void Task_Saf(void *parameter)
{
  ...
  while(1)
  {
    ...
    if(DB.PressVal > 20  && DB.PressVal < 100)
    {
      SafeCount[0] = 1;
    }
    if(DB.Model != 5 || (DB.Model == 5 && TmpFlag == 1))
    {
      if(DB.PressVal > 20 && DB.PressVal < 75)
      {
        SafeCount[1] = 1;
      }
      if(DB.PressVal > 20 && DB.PressVal < 85)
      {
        SafeCount[2] = 1;
      }
      if(DB.PressVal > 20 && DB.PressVal < 95)
      {
        SafeCount[3] = 1;
      }
    }
    if(Tmp != DB.HandVal[1] + DB.HandVal[2] + DB.HandVal[3] + DB.HandVal[4] + DB.HandVal[5])
    {
      if(DB.HandVal[1] + DB.HandVal[2] + DB.HandVal[3] + DB.HandVal[4] + DB.HandVal[5] > Tmp)
      {
        TmpFlag = 1;
      }
      else
      ...
      Tmp = DB.HandVal[1] + DB.HandVal[2] + DB.HandVal[3] + DB.HandVal[4] + DB.HandVal[5];
    }
    if(DB.PressVal > 130 && SafeCount[0] > 0)
    {
      SafeCount[0]++;
    }
    if(DB.PressVal > 80  && SafeCount[1] > 0 && DB.Para[1] == 1)//抓球
    {
      SafeCount[1]++;
    }
    if(DB.PressVal > 90  && SafeCount[2] > 0 && DB.Para[0] == 0)//强度低
    {
      SafeCount[2]++;
    }
    if(DB.PressVal > 100 && SafeCount[3] > 0 && DB.Para[0] == 2)//强度中
    {
      SafeCount[3]++;
    }
    if(DB.Model == 3)  //配置模式关闭动作
    {
      SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;
    }
    if(DB.GymFist == 1)//握拳一阶段关闭动作但做好准备
    {
      SafeCount[0] = 1;SafeCount[1] = 1;SafeCount[2] = 1;SafeCount[3] = 1;
    }
    if(DB.HandAim == 0 && (DB.Model == 5 || DB.Model == 10))//随动伸直下关闭动作
    {
      SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;
    }
    if(DB.Step == 3 && (DB.Model == 4 || DB.Model == 6 || DB.Model == 7 || DB.Model == 8 || DB.Model == 9 || DB.Model == 13 || DB.Model == 15))//被动伸直下关闭动作
    {
      SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;
    }
    if(DB.HelpStep == 0 && (DB.Model == 11 || DB.Model == 12))//语言伸直下关闭动作
    {
      SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;
    }
    if(SafeCount[1] > 10 || SafeCount[2] > 10 || SafeCount[3] > 10)
    {
      if(DB.HandTimer == 1 && (DB.Model == 4 || DB.Model == 6 || DB.Model == 7 || DB.Model == 8 || DB.Model == 9 || DB.Model == 13 || DB.Model == 15))
      {
        SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;//启动一秒内不进行停泵
      }
      else
      {
        Pump(0);
    ...
    {
      SafeCount[0] = 0;SafeCount[1] = 0;SafeCount[2] = 0;SafeCount[3] = 0;
      N_SET(1,1);N_SET(2,1);N_SET(3,1);N_SET(4,1);N_SET(5,1);N_SET(6,1);N_SET(7,1);N_SET(8,1);
      vTaskDelay(HOLD_TIME);
      N_SET(1,2);N_SET(2,2);N_SET(3,2);N_SET(4,2);N_SET(5,2);N_SET(6,2);N_SET(7,2);N_SET(8,2);
    }
  }
}
