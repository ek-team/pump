...
void ADC_Init(void)
{
  ...
  sConfig.Channel = HAND_ADC_DMA_CH;
  sConfig.Rank = HAND_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc)                  //ADC外设初始化配置(HAL库内部调用)
{
  ...
  if(hadc->Instance==ADCx)
  {
    ...
    HAND_ADC_GPIO_CLK_ENABLE();

    GPIO_InitStruct.Pin = PRESS_ADC_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
    HAL_GPIO_Init(PRESS_ADC_GPIO, &GPIO_InitStruct);
		
    GPIO_InitStruct.Pin = HAND_ADC_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
    HAL_GPIO_Init(HAND_ADC_GPIO, &GPIO_InitStruct);
