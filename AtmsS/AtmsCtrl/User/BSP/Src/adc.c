...
void ADC_Init(void)
{
  ...
  sConfig.Channel = PRE1_ADC_DMA_CH;
  sConfig.Rank = PRE1_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
	
  sConfig.Channel = PRE2_ADC_DMA_CH;
  sConfig.Rank = PRE2_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
	
  sConfig.Channel = PRE3_ADC_DMA_CH;
  sConfig.Rank = PRE3_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
	
  sConfig.Channel = PRE4_ADC_DMA_CH;
  sConfig.Rank = PRE4_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
	
  sConfig.Channel = PRE5_ADC_DMA_CH;
  sConfig.Rank = PRE5_ADC + 1;
  HAL_ADC_ConfigChannel(&hadc, &sConfig);
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc)                  //ADC外设初始化配置(HAL库内部调用)
{
  ...
  if(hadc->Instance==ADCx)
  {
    ...
    GPIO_InitStruct.Pin = PRE1_ADC_GPIO_PIN|PRE2_ADC_GPIO_PIN|PRE3_ADC_GPIO_PIN|PRE4_ADC_GPIO_PIN|PRE5_ADC_GPIO_PIN;
