...
//	printf(" %d %d ",DB.Para[27],DB.Para[28]);
    for(uint8_t i=1;i<=5;i++)
    {
      DB.PressValDis[i] = (int16_t)((int32_t)ADC_Val[i]*1597/1250 - 1295);
      DB.PressVal[i] = (int16_t)DB.PressValDis[i]/10;
    }
    DB.PressVal[0] = DB.PressValDis[0]/10;
    DB.PressValDis[0] = 0;
    for(uint8_t i=1;i<=5;i++)
    {
      if(DB.PressValDis[i] > DB.PressValDis[0])
      {
        DB.PressValDis[0] = DB.PressValDis[i];
      }
    }
    vTaskDelay(20);
    ...
    if(DB.PressVal[0] < 70 || (DB.PressVal[0] <= 80 && DB.State == 2))
    {
      SafeCount[0] = 1;SafeCount[1] = 1;SafeCount[2] = 1;SafeCount[3] = 1;
    }
    if(DB.PressVal[0] > 130 && DB.Model != 3) //130Kpa强度高保护
    {
      if(SafeCount[0] > 0) SafeCount[0]++;
    }
    if(DB.PressVal[0] > 80 && DB.Para[1] == 1)//抓球
    {
      if(SafeCount[1] > 0) SafeCount[1]++;
    }
    if(DB.PressVal[0] > 90 && DB.Para[0] == 0)//强度低保护
    {
      if(SafeCount[2] > 0) SafeCount[2]++;
    }
    if(DB.PressVal[0] > 100 && DB.Para[0] == 2)//强度中保护
    {
    ...
    if(DB.PressVal[0] > 60 && DB.Para[6] == 0)//防痉挛启动
    {
